...
class statisticsAll :
    ...
    def callPrcBusAll(self):
        ...
        for key,value in self.dbPortDict.items():
            ...
            try:
                self.cur.callproc('p2')
                print"      p2完成        "
                self.cur.callproc('p_t_weigui')
                print"      p_t_weigui    "
                self.cur.callproc('p3')
                print"      p3完成        "
                self.cur.callproc('p5')
                print"      p5完成        "
                self.cur.callproc('p_15')
                print"      p_15完成      "
                self.cur.callproc('p_16')
                print"      p_16完成      "
                self.cur.callproc('p_17')
                print"      p_17完成      "
                self.cur.callproc('p_18')
                print"      p_18完成      "
                self.cur.callproc('p_19',(time.strftime("%Y-%m-%d", time.localtime(time.time())),))
                print"      p_19完成      "
                self.cur.callproc('p_20',(time.strftime("%Y-%m-%d", time.localtime(time.time())),))
                print"      p_20完成      "
